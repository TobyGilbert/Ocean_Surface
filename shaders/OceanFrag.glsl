...
//  vec3 reflectedCol = skyColour(vec3(reflect(viewVector, vec4(N, 1.0))));
...
//  vec4 lightCol = vec4(9.0/255.0, 176.0/255.0, 170.0/255.0, 1.0);
//  vec4 darkCol = vec4( 22.0/255.0, 80.0/255.0,119.0/255.0, 1.0);
//  fragColour = mix(darkCol, lightCol, (height+80.0)/140.0) + vec4(specular(), 1.0);
//  vec4 refraction = mix(darkCol, lightCol, (height+80.0)/140.0);
//  vec4 globalReflect = vec4(specular(), 1.0);
  fragColour =vec4(vec3(colour), 1.0);///*vec4((1-fresnel)) */ refraction + globalReflect;//vec4(fresnel) * (vec4(sun.colour, 1.0));
